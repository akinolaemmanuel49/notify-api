...
func (r *NotificationRepository) CreateNotification(notification *models.Notification) error {
	...
	_, err := r.db.Exec(query, notification.Title, notification.Message, notification.Priority, notification.CreatedAt, notification.UpdatedAt)
	if err != nil {
...
func (r *NotificationRepository) GetNotificationByID(id int64) (*models.Notification, error) {
	query := `
	SELECT id, title, message, priority, created_at, updated_at FROM notifications WHERE id = ($1)`

	result := r.db.QueryRow(query, id)

	var notification models.Notification
	err := result.Scan(&notification.ID, &notification.Title, &notification.Message, &notification.Priority, &notification.CreatedAt, &notification.UpdatedAt)
	if err != nil {
...
func (r *NotificationRepository) GetAllNotifications(page, pageSize int) ([]*models.Notification, error) {
	...
	query := `
	SELECT id, title, message, priority, created_at, updated_at FROM notifications LIMIT $1 OFFSET $2`
